...
async function toPdf(testCases, project) {
  ...
  testCases.forEach((tc, i) => {
    ...
    doc.text(`${i+1}. ${tc.title}`, 14, y); y += 6
    doc.setFont(undefined, 'normal')
    doc.setFontSize(9)
    doc.text(`Type: ${tc.type}  |  Method: ${tc.methodName || ''}`, 14, y); y += 5
    if (tc.description) { const lines = doc.splitTextToSize(tc.description, 180); doc.text(lines, 14, y); y += lines.length * 4 + 3 }
    y += 4
  })
...
async function toExcel(testCases, project) {
  ...
  const rows = testCases.map(tc => ({ Title: tc.title, Type: tc.type, Method: tc.methodName, Description: tc.description, Code: tc.codeSnippet }))
...
async function screenshot() {
  ...
  if (!el) { alert('No test output visible'); return }
...
export default function ExportBar({ testCases = [], project = 'TestGen AI', framework = '' }) {
  ...
  async function run(id, fn) {
    ...
    try { await fn(); setDone(id); setTimeout(() => setDone(null), 2500) }
    catch (e) { console.error(e) }
    finally { setLoading(null) }
  }

  const exports = [
    { id: 'pdf',   icon: '📄', label: 'PDF',        fn: () => toPdf(testCases, project),   color: '#f87171' },
    { id: 'xl',    icon: '📊', label: 'Excel',      fn: () => toExcel(testCases, project), color: '#4ade80' },
    { id: 'img',   icon: '📸', label: 'Screenshot', fn: screenshot,                        color: '#22d3ee' },
  ]

  function shareTitle() {
    if (disabled)                        return 'Generate test cases first'
    if (blocked === 'cooldown')          return `Cooldown active — ${timeLeft} remaining`
    if (blocked === 'daily_limit')       return 'Daily limit reached — resets at midnight'
        ...
        >
          ...
          {!isPaid && !disabled && (
            <span className={`${s.pill} ${!canShare ? s.pillWarn : ''}`}>
              {canShare ? `${left} left` : blocked === 'cooldown' ? timeLeft : 'midnight'}
            </span>
          )}
        </button>
