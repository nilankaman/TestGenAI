    ...
    <div className={s.overlay} ref={ref} onClick={overlayClick}>
      <div className={s.modal}>
        <div className={s.head}>
          <div className={s.headLeft}>
            ...
            <div>
              ...
              <p className={s.sub}>
                {project || "TestGen AI"} · {testCases.length} test cases
              </p>
            </div>
          </div>
        ...
        {!isPaid && (
          <div className={s.usageBar}>
            ...
            <div className={s.track}>
              <div
                className={`${s.fill} ${used >= dailyLimit ? s.fillFull : ""}`}
                style={{ width: `${barPct}%` }}
              />
            </div>
            <p className={s.usageMeta}>
              {inCooldown && cooldownEnd ? (
                <>
                  Next share in <Countdown ms={cooldownEnd} />
                </>
              ) : left > 0 ? (
                `${left} left — resets at midnight`
              ) : (
          ...
          <div className={s.blocked}>
            ...
            <div>
              ...
              <p className={s.blockedDesc}>
                {blocked === "cooldown"
                  ? `Available again in ${timeLeft}. Free plan: ${dailyLimit}/day, 6hr cooldown.`
                  : `All ${dailyLimit} shares used today. Resets at midnight.`}
              </p>
            </div>
            <button
        ...
        <div className={`${s.body} ${!canShare ? s.dim : ""}`}>
          <div className={s.section}>
            ...
            <div className={s.types}>
              {types.map((t) => (
                <button
                  key={t.id}
                  className={`${s.typeBtn} ${fileType === t.id ? s.typeBtnOn : ""}`}
          ...
          <div className={s.section}>
            <p className={s.sLabel}>
              Send to{picked.length > 0 ? ` · ${picked.length} selected` : ""}
            </p>
            <div className={s.members}>
              {team.map((m) => {
                ...
                return (
                  <button
                    key={m.id}
                    className={`${s.member} ${on ? s.memberOn : ""}`}
                    onClick={() => toggle(m.id)}
                    disabled={!canShare}
                  >
                    <span className={s.mAvatar} style={{ background: m.color }}>
                      {m.avatar}
                      {m.online && <span className={s.mDot} />}
                    </span>
                    <div className={s.mInfo}>
                      <span className={s.mName}>{m.name}</span>
                      <span className={s.mEmail}>{m.email}</span>
                    </div>
        ...
        <div className={s.foot}>
          {!isPaid && (
            <p className={s.footNote}>
              Free plan ·{" "}
              <a href="/subscription" className={s.upLink} onClick={onClose}>
          ...
          <div className={s.footRight}>
            ...
            <button
              className={`${s.sendBtn} ${done ? s.sendDone : ""}`}
              onClick={send}
              disabled={!canShare || picked.length === 0 || busy}
            >
              {done ? (
                " Sent!"
              ) : busy ? (
                ...
                `Share ${chosen?.icon} with ${picked.length || "…"}`
              )}
            </button>
          </div>
        </div>
