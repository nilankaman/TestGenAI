...
export default function Topbar({ onMenuClick, menuOpen }) {
  ...
  const themeTitle = { dark: 'Dark mode — click for Light', light: 'Light mode — click for System', system: 'System mode — click for Dark' }
        ...
        <div className={s.avatarWrap} ref={dropRef}>
          ...
          {dropOpen && (
            <div className={s.drop}>
              ...
              <button className={s.dropItem} onClick={() => go('/profile')}>      👤 Profile     </button>
              <button className={s.dropItem} onClick={() => go('/settings')}>     ⚙️ Settings    </button>
              <button className={s.dropItem} onClick={() => go('/subscription')}> 💎 Plan        </button>
              <button className={s.dropItem} onClick={() => go('/portfolio')}>    🗂️ Portfolio   </button>
              <div className={s.divider} />
              <button className={`${s.dropItem} ${s.dropRed}`} onClick={handleLogout}>
                🚪 Sign out
              </button>
            </div>
          )}
