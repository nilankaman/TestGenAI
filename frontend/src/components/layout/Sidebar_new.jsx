...
const nav = [
  ...
  { to: '/cicd-generator', icon: '🔄', key: 'cicd',    badge: 'NEW', badgeColor: 'green'  },
  { to: '/search',         icon: '🔍', key: 'search'         },
  { to: '/subscription',   icon: '💎', key: 'plan',    badge: 'PLAN', badgeColor: 'plan'  },
]
...
export default function Sidebar({ open, onNav }) {
  ...
  const isAdmin = localStorage.getItem('tg-token') === 'admin-token-2024-testgen'

  function go(path) { navigate(path); onNav?.() }
  function switchLang(code) { i18n.changeLanguage(code); localStorage.setItem('testgen-lang', code) }

  function badgeLabel(item) {
    if (item.badgeColor !== 'plan') return item.badge
    return isPaid ? plan.charAt(0).toUpperCase() + plan.slice(1) : `${left} left`
  }
          ...
          {nav.map(item => (
            <li key={item.to}>
              ...
              >
                ...
                {item.badge && (
                  <span className={`${s.badge} ${s[`badge_${item.badgeColor}`]}`}>
                    {badgeLabel(item)}
                  </span>
                )}
              </NavLink>
        ...
        <div className={s.planCard} onClick={() => go('/subscription')}>
          <div className={s.planRow}>
            <span className={s.planName}>
              {isPaid ? `💎 ${plan.charAt(0).toUpperCase() + plan.slice(1)}` : '💎 Free Plan'}
            </span>
            <span className={s.planArrow}>{isPaid ? '' : '→'}</span>
          </div>
          <p className={s.planDesc}>
            {isPaid ? 'Unlimited sharing · No cooldown' : `${left} share${left !== 1 ? 's' : ''} left today`}
          </p>
          {!isPaid && (
            <>
              <div className={s.planBar}>
                <div className={s.planFill} style={{ width: `${Math.min((used / dailyLimit) * 100, 100)}%` }} />
              </div>
              <span className={s.planMeta}>{used} / {dailyLimit} used today</span>
            </>
          )}
        </div>
      </nav>
