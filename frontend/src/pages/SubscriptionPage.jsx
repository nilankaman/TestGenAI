...
const PLANS = [
  ...
  {
    id: 'pro', name: 'Pro', price: '¥1,490', period: '/month', color: '#7c6cfa', badge: 'Most Popular',
    features: [
  ...
  {
    id: 'team', name: 'Team', price: '¥3,980', period: '/month', color: '#22d3ee',
    features: [
...
function HistoryRow({ entry }) {
  ...
  const icon = { pdf: '📄', screenshot: '📸', excel: '📊' }[entry.fileType] || '📤'
  return (
    <div className={s.hRow}>
      <span>{icon}</span>
      <div className={s.hInfo}>
        <span className={s.hProject}>{entry.project || 'Untitled'}</span>
        <span className={s.hMeta}>{entry.testCount || 0} tests · {(entry.fileType || '').toUpperCase()} · {entry.recipients?.length || 0} recipients</span>
      </div>
      <span className={s.hTime}>{day} {time}</span>
    </div>
  )
}
...
export default function SubscriptionPage() {
  ...
  const { plan, isPaid, canShare, left, used, dailyLimit, blocked, timeLeft, cooldownEnd, history } = useShareStore()
      ...
      <div className={s.inner}>
        ...
        <div className={s.usageCard}>
          <div className={s.usageLeft}>
            ...
            <div>
              ...
              <p className={s.usageSub}>{isPaid ? 'Unlimited — no cooldown' : `Free plan · resets at ${midnight.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`}</p>
            </div>
          </div>
          {!isPaid && (
            <div className={s.usageRight}>
              <span className={s.usageNum}>{used}</span><span className={s.usageDen}>/ {dailyLimit}</span>
              <div className={s.usageBarWrap}>
                <div className={s.usageTrack}>
                  <div className={`${s.usageFill} ${used >= dailyLimit ? s.usageFull : ''}`} style={{ width: `${barPct}%` }} />
                </div>
              </div>
            </div>
          )}
        ...
        {!isPaid && (
          <div className={`${s.alert} ${blocked ? s.alertWarn : s.alertOk}`}>
            <span>{blocked === 'cooldown' ? '⏳' : blocked === 'daily_limit' ? '🚫' : '✅'}</span>
            <span className={s.alertText}>
              {blocked === 'cooldown'    && <><strong>6-hour cooldown active.</strong> Next share in <strong>{timeLeft}</strong>.</>}
              {blocked === 'daily_limit' && <><strong>Daily limit reached.</strong> Resets at midnight.</>}
              {!blocked                  && <><strong>{left} share{left !== 1 ? 's' : ''} remaining</strong> today.</>}
            </span>
          </div>
        )}
        ...
        <div className={s.historyCard}>
          ...
          {history.length === 0
            ? <div className={s.historyEmpty}><span>📭</span><p>Nothing shared today.</p></div>
            : <div>{[...history].reverse().map((e, i) => <HistoryRow key={i} entry={e} />)}</div>
          }
        </div>

        {/* Plans */}
        <h2 className={s.sectionTitle} style={{ marginTop: 28 }}>Plans</h2>
          ...
          {PLANS.map(p => {
            ...
            return (
              <div
                ...
                style={{ '--pc': p.color }}
              >
                {p.badge && <div className={s.planBadge} style={{ background: p.color }}>{p.badge}</div>}
                {isCurrent && <div className={s.currentTag}> Active</div>}
                <h3 className={s.planName} style={{ color: p.color }}>{p.name}</h3>
                ...
                <ul className={s.planFeatures}>
                  {p.features.map((f, i) => (
                    <li key={i} className={`${s.planFeature} ${!f.on ? s.planFeatureOff : ''}`}>
                      <span className={f.on ? s.checkOn : s.checkOff}>{f.on ? '' : '✗'}</span>
                      {f.text}
                    </li>
                  ))}
                </ul>
                <button
                  ...
                  style={!isCurrent && p.id !== 'free' ? { background: p.color } : {}}
                  onClick={() => {
                    if (isCurrent) return
                    if (p.id === 'free') return
        ...
        <p className={s.disclaimer}>
          This is a portfolio demo. The payment page is a mock — no real charges occur.
        </p>
      </div>
    </div>
  )
