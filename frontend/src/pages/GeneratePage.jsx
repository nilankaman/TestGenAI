...
export default function GeneratePage() {
  ...
  async function buildScript() {
    if (!desc.trim()) {
      setError("Describe the feature first.");
      return;
    }
    setError("");
    setScriptCode("");
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          messages: [
            { role: "user", content: buildScriptPrompt(scriptFw, desc) },
          ],
        }),
      });
      const data = await res.json();
      ...
      setError(
        e.message || "Script generation failed. Is the backend running?",
      );
    } finally {
      ...
      <div className={s.right} id="test-output">
        ...
        {tab === "script" && scriptCode && !scriptLoading && (
          <div className={s.scriptOutput}>
            ...
            <pre className={s.scriptPre}>
              <code>{scriptCode}</code>
            </pre>
            <div className={s.scriptFooter}>
              <p className={s.scriptNote}>
                ✅ Ready to use — copy into your project and update
                selectors/URLs for your app.
              </p>
              <button
        ...
        {tab === "manual" && results && !loading && (
          <div className={s.results}>
            <div className={s.resultsHeader}>
              <div>
                <h2 className={s.resultsTitle}>
                  {results.testCases?.length} manual test cases
                </h2>
                <p className={s.resultsMeta}>
        ...
        {tab === "auto" && results && !loading && (
          <div className={s.results}>
            <div className={s.resultsHeader}>
              <div>
                <h2 className={s.resultsTitle}>
                  {results.testCases?.length} automation test cases
                </h2>
                <p className={s.resultsMeta}>
                  {results.framework} · Coverage: {results.coverageScore}%
                </p>
              </div>
              <div className={s.scoreRing}>{results.coverageScore}%</div>
            </div>
