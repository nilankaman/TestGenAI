...
const ALLOWED_EXTENSIONS = ['.pdf','.png','.jpg','.jpeg','.gif','.webp','.xlsx','.xls','.csv','.txt','.zip']
...
const MOCK_FILES = [
  { id: 1, name: 'login-test-report.pdf',  size: '248 KB', type: 'pdf',   date: '2 days ago', shared: 3 },
  { id: 2, name: 'checkout-screenshot.png',size: '1.2 MB', type: 'image', date: '3 days ago', shared: 2 },
  { id: 3, name: 'regression-results.xlsx',size: '96 KB',  type: 'excel', date: '1 week ago', shared: 5 },
]
...
export default function AttachmentsPage() {
  ...
  function shareFile(id) {
    setFiles(prev => prev.map(f => f.id === id ? { ...f, shared: f.shared + 1 } : f))
  ...
  return (
    <div className={s.page}>
      <div className={s.header}>
        ...
        <p className={s.sub}>Upload test reports, screenshots, and evidence files to share with your team.</p>
      </div>
      ...
      >
        ...
        <p className={s.dropSub}>PDF, PNG, JPG, XLSX, CSV, TXT, ZIP · max 20 MB each</p>
        <button
        ...
        <div className={s.stagingArea}>
          <div className={s.stagingHeader}>
            <div>
              ...
              <p className={s.stagingSub}>{staged.length} file{staged.length > 1 ? 's' : ''} queued — review before submitting</p>
            </div>
            <button className={s.clearBtn} onClick={clearStaged}>Clear all</button>
          </div>
            ...
            {staged.map(file => (
              <div key={file.id} className={s.stagedRow}>
                <div className={s.stagedLeft}>
                  {file.preview ? (
                    <img src={file.preview} alt={file.name} className={s.previewThumb} />
                  ) : (
                    <div
                      className={s.stagedIconWrap}
                      style={{ background: `${TYPE_COLOR[file.type]}18`, color: TYPE_COLOR[file.type] }}
                    >
                      {ICON[file.type]}
                    </div>
                  )}
                  <div className={s.stagedInfo}>
                    ...
                    <p className={s.stagedMeta}>{file.size} · {file.type}</p>
                  </div>
                </div>
                <button
          ...
          <div className={s.stagingActions}>
            <button className={s.cancelBtn} onClick={clearStaged} disabled={uploading}>
              Cancel
            </button>
            <button className={s.uploadBtn} onClick={submitFiles} disabled={uploading}>
              {uploading
          ...
          {filtered.map(file => (
            <div key={file.id} className={s.fileRow}>
              ...
              <div className={s.fileActions}>
                <button className={s.shareFileBtn} onClick={() => shareFile(file.id)}>
                  📤 Share
                </button>
                <button className={s.downloadBtn}>
                  ⬇ Download
                </button>
                <button className={s.deleteBtn} onClick={() => deleteFile(file.id)}>
                  🗑
                </button>
              </div>
            </div>
