  ...
  return (
    <div className={styles.page}>
        ...
        <div className={styles.headerLeft}>
          ...
          <div>
            ...
            <p className={styles.sub}>
              Monitor your test pipelines and trigger runs directly from TestGen
              AI.
            </p>
          </div>
        </div>
      ...
      <div className={styles.pipelineList}>
        {MOCK_PIPELINES.map((p) => (
          <div key={p.id} className={styles.pipelineCard}>
            <span className={styles.pipelineStatusIcon}>
              {STATUS_ICON[p.status]}
            </span>
            <div className={styles.pipelineInfo}>
              <p className={styles.pipelineName}>{p.name}</p>
              <p className={styles.pipelineMeta}>
                Triggered by <strong>{p.trigger}</strong> · {p.time} · took{" "}
                {p.duration}
              </p>
            </div>
            <div className={styles.pipelineRight}>
              <span
                className={`${styles.pill} ${styles[`pill_${STATUS_COLOR[p.status]}`]}`}
              >
                {p.status}
              </span>
      ...
      <div className={styles.mockNote}>
        ⚠️ This is a mock UI. Connect GitHub Actions, Jenkins, or GitLab CI to
        see real pipelines.
      </div>
    </div>
  );
